/**
 * wave-effect.ultimate-optimized.js (micro-tuned version, resource enhanced, lightweight scroll-aware)
 * - ประหยัดทรัพยากร, ultra smooth, คุณภาพเอฟเฟกต์เหมือนเดิม 100%
 * - ป้องกัน ripple ระหว่าง scroll
 * - เพิ่ม CSS contain, ลด will-change, ลด global ripple
 ...
 * - ลด observer scope และ DOM access
 * - Passive event, non-blocking scroll
 * - ฟังก์ชันและเอฟเฟกต์ครบ
 */
(function(root, factory) {
  ...
  const GLOBAL_MAX_RIPPLES = 4; // ลดจาก 7 เป็น 4
  ...
  function getRippleNode(el) {
    ...
    if (!node) {
      ...
      node.style.contain = 'layout style'; // เพิ่ม containment
    }
    node.classList.remove('animating','fading');
    node.style.cssText = '';
    node.style.contain = 'layout style'; // reset containment
    return node;
  }
  ...
  // เพิ่มตัวแปรสำหรับตรวจจับ scroll
  let isScrolling = false;
  let scrollTimer = null;
  window.addEventListener('scroll', () => {
    isScrolling = true;
    clearTimeout(scrollTimer);
    scrollTimer = setTimeout(() => { isScrolling = false; }, 170);
  }, {passive: true});
  ...
  function fadeOutAndRemoveRipple(ripple, fadeType, el) {
    ...
    function removeRippleAfterFade(e) {
      ...
      if (!e || e.propertyName === "opacity") {
        ...
        ripple.style.removeProperty('will-change'); // ถอด will-change เร็วที่สุด
  ...
  function onPointerDown(e) {
    if (isScrolling) return; // หลีกเลี่ยง ripple ระหว่าง scroll
  ...
  // Observer: เฉพาะ childList + attributes, ไม่ observe subtree ลึก
  let observer;
  function observeDom() {
    if (observer) return;
        ...
        if (m.type === 'childList') {
          m.addedNodes && m.addedNodes.forEach(n=>{
            if (n.nodeType === 1 && n.hasAttribute && n.hasAttribute('wave')) {
              ...
              n.querySelectorAll('[wave]').forEach(el=>upgradeElement(el));
            }
          });
        } else if (m.type === 'attributes' && m.attributeName === 'wave' && m.target) {
    ...
    observer.observe(document.body, {
      ...
      childList: true,
      attributes: true,
      attributeFilter: ['wave','data-ripple-color']
    });
  }
