/**
 * wave-effect.ultra-optimized.js
 * - Ultra smooth, ultra resource-saving, 100% effect quality preserved
 * - All work batched in requestAnimationFrame
 * - Ripple pool per element (reuse node, limit max per element)
 * - will-change only during animate, removed after fade
 * - Observer scope and DOM access minimized
 * - Passive event, non-blocking scroll
 * - Full features and effect
 */
...
(function(root, factory) {
  ...
  // Ripple queue & rAF batcher (Set to auto-merge jobs)
  const rippleQueue = new Set();
  let rippleRAF = null;
  // Prevent new ripple burst during scrolling
  ...
  // Observer: childList only, debounce 200ms, no subtree, no attribute watch
  let observer;
  function observeDom() {
    if (observer) return;
