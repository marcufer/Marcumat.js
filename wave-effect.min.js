/**
 * wave-effect.ultra-lite.js
 * - เบา ลื่น ลด resource ระหว่าง scroll, คุณภาพ effect เดิม
 * - ไม่ก่อให้เกิด ripple ระหว่าง scroll หรือทันทีหลัง scroll
 * - ลด timer/DOM access/observer debounce
 * - ฟีเจอร์ & คุณภาพเท่าเดิม
 */
(function(root, factory) {
  ...
  // Ripple pool per element (limit MAX_RIPPLES_PER_ELEMENT)
  ...
  // --- SCROLL BLOCK LOGIC (ปรับปรุงใหม่) ---
  let scrollActive = false, scrollBlockUntil = 0, scrollBlockTimeout = null;
  window.addEventListener('scroll', () => {
    scrollActive = true;
    scrollBlockUntil = Date.now() + 120;
    if (scrollBlockTimeout) clearTimeout(scrollBlockTimeout);
    scrollBlockTimeout = setTimeout(() => { scrollActive = false; }, 140);
  }, {passive:true});
  function allowRippleNow() {
    return !scrollActive && Date.now() > scrollBlockUntil;
  }
  // -----------------------------------------
  ...
  // Observer: childList only, debounce 160ms, no subtree, no attribute watch
  let observer;
  function observeDom() {
    if (observer) return;
