...
(function(root, factory) {
  ...
  let RIPPLE_FADE_DURATION_AUTO = 1000;   // ผู้ใช้แตะแล้วยกออกทันที (auto-fade)
  let RIPPLE_FADE_DURATION_MANUAL = 600; // ผู้ใช้แตะค้างจน animation จบ แล้วยกนิ้วออก (manual-fade)

  // --- Ripple geometry ---
  const RIPPLE_CENTER_DIAMETER = 18;
  const RIPPLE_HALO_START_DIAMETER = 18;
  const RIPPLE_HALO_BLUR = 0.8;
  const COVERAGE_EXPAND_RATIO = 0.3;
  ...
  // Only load from /assets/wave-effect.min.css (single location)
  ...
  // Ripple color and opacity control
  function getRippleBg(color) {
    ...
    if (color) {
      if (/^rgba?\([^)]+\)$/i.test(color) || /^hsla?\([^)]+\)$/i.test(color)) {
        ...
        if (inside) {
          ...
          let p1 = Math.min(1, Math.max(alpha, 0.13)); // core
          let p2 = Math.max(0, 0.6 * p1);              // halo
          let p3 = Math.max(0, 0.09 * p1);             // edge
    ...
    return `radial-gradient(circle, rgba(${defaultC[0]},${defaultC[1]},${defaultC[2]},0.13) 62%, rgba(${defaultC[0]},${defaultC[1]},${defaultC[2]},0.078) 74%, rgba(${defaultC[0]},${defaultC[1]},${defaultC[2]},0.02) 100%)`;
  }
  ...
  // Fade-out logic: duration แบบแยกกรณี
  function fadeOutAndRemoveRipple(ripple, fadeType /* 'auto' | 'manual' */) {
    if (!ripple) return;
  ...
  function clearRipples(el) {
    ...
    ripples.forEach(r => {
      fadeOutAndRemoveRipple(r, 'manual');
    });
  ...
  function onPointerDown(e) {
    ...
    const rippleCreatedAt = performance.now();
    ...
    function endRipple(){
      ...
      if (!animationEnded) {
        // กรณีผู้ใช้วางนิ้วลงแล้วยกออกทันที (auto)
        fadeOutAndRemoveRipple(ripple, 'auto');
      } else {
        // กรณี manual (ผู้ใช้ค้างจน animation จบ แล้วปล่อย)
    ...
    // หากผู้ใช้วางนิ้วค้างจน animation จบ แล้วยังไม่ปล่อย ให้รอ pointerReleased ก่อนถึงจะ fade
    autoFadeTimeout = setTimeout(() => {
      if (!pointerReleased && ripple.parentNode) {
        // animation จบแล้วแต่ pointer ยังไม่ถูกปล่อย, ยังไม่ fade รอ pointerup
        // (nothing)
      }
    }, duration + 30);
  }
  ...
  function upgradeAll() {
    document.querySelectorAll('[wave]').forEach(upgradeElement);
