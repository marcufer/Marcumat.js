/**
 * wave-effect.js
 * Default ripple color: deeper dark, slightly blue-black (more "platform-like"), with increased opacity for better visibility.
 */
...
(function(root, factory) {
  ...
  function getRippleBg(color) {
    // Default: deeper, more neutral black with a subtle blue hint (platform-like). Example: rgb(16,20,28)
    const defaultC = [16, 20, 28]; // #10141C (deep black, a bit blue but not too much)
    if (color) {
      if (/^rgba?\([^)]+\)$/i.test(color) || /^hsla?\([^)]+\)$/i.test(color)) {
        ...
        if (inside) {
          ...
          // Slightly more visible than before and platform-like
    ...
    // Default: deep black with a subtle blue, and more visible
    return `radial-gradient(circle, rgba(${defaultC[0]},${defaultC[1]},${defaultC[2]},0.13) 62%, rgba(${defaultC[0]},${defaultC[1]},${defaultC[2]},0.078) 74%, rgba(${defaultC[0]},${defaultC[1]},${defaultC[2]},0.012) 100%)`;
  }
