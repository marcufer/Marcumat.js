/**
 * wave-effect.ultra-lite.v6.js (tuned for much slower expansion + much slower fade)
 *
 * Changes in this version:
 * - Substantially slower expansion: INTERNAL_SPEED_MULTIPLIER increased to multiply transform durations heavily.
 * - Much slower fade: RIPPLE_FADE_DURATION_AUTO/MANUAL increased to give a long, natural fade-out.
 * - Blur ramp scales with duration but is clamped to avoid excessively expensive blur ramps.
 * - Retains performance tiers and prefers-reduced-motion respect.
 */
(function (root, factory) {
  ...
  const BASE_DURATION = 550;
  const MIN_DURATION = 550;
  const MAX_EXTRA = 0.1;
  const RIPPLE_HALO_START_DIAMETER = 18;
  // tuned blur: reduced to be visible but cheaper; small float yields smooth GPU blur
  const RIPPLE_HALO_BLUR = 0.6;
  const COVERAGE_EXPAND_RATIO = 1.8;
  // Limit default simultaneous ripples per element to 2 to reduce peak paint
  const MAX_RIPPLES_PER_ELEMENT = 2;
  // Increased heavily to make expansion substantially slower (higher = slower)
  const INTERNAL_SPEED_MULTIPLIER = 11.0;
  // Much slower fades (more gradual disappearance)
  let RIPPLE_FADE_DURATION_AUTO = 1400;
  let RIPPLE_FADE_DURATION_MANUAL = 1000;
  ...
  // --- Performance detection & adaptive tuning ---
  function detectPerformanceLevel() {
    try {
      const cores = (navigator && navigator.hardwareConcurrency) ? navigator.hardwareConcurrency : 4;
      const mem = (navigator && navigator.deviceMemory) ? navigator.deviceMemory : 4;
      const ua = (navigator && navigator.userAgent) ? navigator.userAgent : '';
      // Heuristics:
      // - low: <=2 cores OR <=1.5GB memory OR old Android (<=8) in UA
      // - medium: <=4 cores OR <=3GB memory
  ...
  // Add root class so CSS can adjust via variables too
  try {
  ...
  function getInternalSpeedMultiplierLocal() {
    if (PERF_LEVEL === 'low') return Math.max(1.6, INTERNAL_SPEED_MULTIPLIER * 0.6);
    if (PERF_LEVEL === 'medium') return INTERNAL_SPEED_MULTIPLIER * 0.85;
    return INTERNAL_SPEED_MULTIPLIER;
  }
  function getHaloBlurLocal() {
    if (PERF_LEVEL === 'low') return 0;
    if (PERF_LEVEL === 'medium') return RIPPLE_HALO_BLUR * 0.6;
    return RIPPLE_HALO_BLUR;
  }
  ...
  function computeGradient(color) {
    ...
    // adaptive TTL: keep gradients longer on low-end to avoid recompute
    ...
    if (m) {
      ...
      // Keep ripple alpha reasonably visible
  ...
  function getDefaultRippleColor(el) {
    ...
    const t = now();
    // longer cache on low end
    const localTTL = PERF_LEVEL === 'low' ? DEFAULT_COLOR_CACHE_TTL * 4 : DEFAULT_COLOR_CACHE_TTL;
  ...
  function onPointerDown(e) {
    ...
    // Use adaptive internal multiplier
    const internalMul = getInternalSpeedMultiplierLocal();
    const scaledDuration = Math.max(120, Math.round(baseDuration * internalMul * getExternalDurationMultiplier()));
    ...
    // Use a proportion that scales with the new slower durations, but clamp to avoid very expensive blur ramps
    const rawBlur = Math.max(80, Math.round(scaledDuration * 0.45));
    const blurDuration = Math.min(600, rawBlur);
    ...
    // Adaptive blur: for low-end devices this returns 0
    const effectiveBlur = prefersReduced ? 0 : getHaloBlurLocal();

    schedule(function () {
      // Limit number of simultaneous ripples per element (fade oldest if needed)
      let set = activeRipples.get(el);
      if (!set) { set = new Set(); activeRipples.set(el, set); }
      if (set.size >= getMaxRipples()) {
        // Fade oldest ripple to free capacity (prefer to fade rather than drop entirely)
        try {
          const it = set.values().next();
          if (!it.done && it.value) fadeOutAndRemoveRipple(it.value, 'manual', el);
        } catch (e) { /* ignore */ }
      }
      ...
      // Also adapt box-shadow on low-end to reduce paint cost
      ...
      ripple.style.cssText =
        ...
        'background:' + bg + ';transform:scale(1) translate3d(0,0,0);backface-visibility:hidden;contain:strict;box-shadow:' + boxShadow + ';';

      el.appendChild(ripple);
      let setNow = activeRipples.get(el);
      if (!setNow) { setNow = new Set(); activeRipples.set(el, setNow); }
      setNow.add(ripple);
      ...
      function endRipple() {
        ...
        try { if (setNow && setNow.delete) setNow.delete(ripple); } catch (e) { }
        removeListeners();
      }
      function removeListeners() {
  ...
  function onKeyDown(e) {
    ...
    schedule(function () {
      ...
      const scaledDuration = Math.max(120, Math.round(baseDuration * getInternalSpeedMultiplierLocal() * getExternalDurationMultiplier()));
      const colorVal = getRippleColor(el);
      const bg = computeGradient(colorVal);
      const ripple = getRippleNode(el);
      const size = RIPPLE_HALO_START_DIAMETER + 'px';
      ...
      const rawBlur = Math.max(80, Math.round(scaledDuration * 0.42));
      const blurDuration = Math.min(600, rawBlur);
