/**
 * Further optimized wave-effect.js
 * - ลดการใช้ทรัพยากรลงอีก โดยยังคงคุณภาพเดิม 100% และฟังก์ชันครบทุกอย่าง
 * - จุดที่ปรับเพิ่มจากเวอร์ชันล่าสุด:
 *   - Debounce upgradeAll, ใช้ WeakSet cache element ที่ upgrade แล้ว
 *   - ไม่ disconnect observer อีก (ลด overhead)
 *   - ใช้ passive:true กับ event ทั้งหมด
 *   - ใช้ requestIdleCallback กับงานเบา/upgrade ไม่เร่งด่วน
 *   - ปรับการ inject CSS/style ให้แน่ใจว่าไม่ซ้ำ
 *   - ไม่มีผลกระทบกับ UI/สี/ฟีล/animation เดิม
 */
(function(root, factory) {
  ...
  // cache สำหรับ element ที่ upgrade แล้ว
  ...
  // -- upgradeElement ใช้ WeakSet กัน re-upgrade และเบาขึ้น
  ...
  // --- Debounce/idle upgradeAll ---
  let upgradeAllPending = false;
  function upgradeAll() {
    if (upgradeAllPending) return;
    upgradeAllPending = true;
    if ('requestIdleCallback' in window) {
      requestIdleCallback(() => {
        const nodes = querySelectorAllCached('[wave]');
        for (let el of nodes) upgradeElement(el);
        if (settings.disableTapHighlight) injectTapHighlightCSS();
        upgradeAllPending = false;
      }, {timeout: 200});
    } else {
      setTimeout(()=>{
        const nodes = querySelectorAllCached('[wave]');
  ...
  // Event delegation: ติด global handler ทีเดียว
  ...
  // Observer: ไม่ disconnect อีกแล้ว (ตลอด session)
  let observer;
  function observeDom() {
    if (observer) return;
    observer = new MutationObserver(function(muts){
      for (const m of muts) {
        if (m.type === 'childList') {
          m.addedNodes && m.addedNodes.forEach(n=>{
            if (n.nodeType === 1 && n.hasAttribute && n.hasAttribute('wave')) {
              upgradeElement(n);
            }
            if (n.querySelectorAll) n.querySelectorAll('[wave]').forEach(el=>{
              upgradeElement(el);
            });
          });
      ...
      // ไม่ disconnect observer เพื่อความต่อเนื่อง
    });
