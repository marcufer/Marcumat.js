/**
 * wave-effect.ultimate-optimized.js
 * - สำหรับงานเฟรมเรตสูง (90+ fps), ประหยัดทรัพยากร, ลื่นไหล ไม่ลดคุณภาพ UI/Animation/Fx
 * - Batch งานทั้งหมดใน requestAnimationFrame
 * - ใช้ ripple pool แบบ per-element (จำกัดจำนวน)
 * - will-change เฉพาะตอน animate แล้วถอดออกเมื่อ fade จบ
 * - ลด observer scope และป้องกัน DOM access บ่อย
 * - Passive event, ไม่ blocking scroll
 * - ฟังก์ชันและ UI effect เดิม 100%
 */
(function(root, factory) {
  ...
  function querySelectorAllCached(sel) {
    ...
    if (!c || c.sel !== sel || (Date.now() - c.stamp > 3000)) {
  ...
  // Ripple pool per element (จำกัด MAX_RIPPLES_PER_ELEMENT)
  ...
  // Observer: observe เฉพาะ childList + attributes, ไม่ observe subtree ลึกเกินไป
  let observer;
  function observeDom() {
    if (observer) return;
