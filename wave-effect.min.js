/**
 * wave-effect.optimized.js
 * - เน้นประหยัดทรัพยากร, UI/คุณภาพ/ฟังก์ชันต้องเหมือนเดิม 100%
 * - เพิ่มประสิทธิภาพ: ใช้ pool สำหรับ ripple, cache selector, event passive, debounce, requestIdleCallback
 * - ลด GC, DOM access, memory leak, animation ลื่นขึ้น
 * - ไม่เปลี่ยนแปลง CSS หรือ API เดิม เพื่อให้ UI/ฟังก์ชันครบ
 */
(function(root, factory) {
  ...
  // Cache: element ที่ upgrade แล้ว
  ...
  function querySelectorAllCached(sel) {
    ...
    if (!c || c.sel !== sel || (Date.now() - c.stamp > 3000)) { // 3s cache
  ...
  // Ripple node pool
  const ripplePool = [];
  function getRippleNode() {
    let node = ripplePool.pop();
    if (!node) {
  ...
  function releaseRippleNode(node) {
    node.remove();
