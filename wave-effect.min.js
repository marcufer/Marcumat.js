/**
 * wave-effect.ultra-optimized.js (improved for minimal resource & ultra smoothness)
 * - All work batched in requestAnimationFrame or requestIdleCallback
 ...
 * - IntersectionObserver for upgrade only visible elements
 * - DOM access and getBoundingClientRect minimized
  ...
  function scheduleRipple(fn) {
    ...
    if (!rippleRAF) {
      rippleRAF = requestAnimationFrame(() => {
        if (performance.now() - lastScroll < 120) {
          setTimeout(() => {
