/**
 * wave-effect.js
 * Enhanced: Separated fade-out duration for easier developer customization and smoother removal logic.
 */
...
(function(root, factory) {
  ...
  // --- Fade-out duration config (customizable by developer) ---
  let RIPPLE_FADE_DURATION = 500; // default fade-out duration in ms
  ...
  // Core: fade-out logic separated
  function fadeOutAndRemoveRipple(ripple) {
    if (!ripple) return;
    ...
    // Fallback in case transitionend not fired (e.g. display:none, or not in DOM)
    setTimeout(removeRippleAfterFade, RIPPLE_FADE_DURATION + 200);
  }

  function clearRipples(el) {
    const ripples = el.querySelectorAll('.' + RIPPLE_CLASS);
    ripples.forEach(r => {
      fadeOutAndRemoveRipple(r);
    });
  ...
  function onPointerDown(e) {
    ...
    // Auto-fade when animation done (user holds down or never releases)
    setTimeout(() => {
      if (!animationEnded && ripple.parentNode) {
        fadeOutAndRemoveRipple(ripple);
        activeRipples.set(el, null);
        el.removeEventListener('pointerup', endRipple);
        el.removeEventListener('pointerleave', endRipple);
  ...
  // Allow developer to set fade-out duration globally via WakeEffect.config.setFadeDuration(ms)
  const config = {
    BASE_DURATION,
    MIN_DURATION,
    MAX_EXTRA,
    ...
    setFadeDuration(ms) {
      if (typeof ms === 'number' && ms > 50 && ms < 5000) RIPPLE_FADE_DURATION = ms|0;
    }
  };
