 ...
 * @version 2.3.5
 * (improved: ripple color and opacity now controlled only in JS, default is just slightly less transparent than before)
 */
...
(function(root, factory) {
  ...
  // Ripple color and opacity control (default: slightly less transparent than before)
    ...
    if (color) {
      if (/^rgba?\([^)]+\)$/i.test(color) || /^hsla?\([^)]+\)$/i.test(color)) {
        ...
        if (inside) {
          ...
          // Slightly less transparent than previous version
          let p1 = Math.min(1, Math.max(alpha, 0.075)); // core
          let p2 = Math.max(0, 0.5 * p1);               // halo
          let p3 = Math.max(0, 0.066 * p1);             // outer edge
      ...
      return `radial-gradient(circle, ${color}13 62%, ${color}09 74%, ${color}01 100%)`;
    }
    // default system ripple (slightly less transparent blue-gray)
    return `radial-gradient(circle, rgba(24,68,102,0.075) 62%, rgba(24,68,102,0.037) 74%, rgba(24,68,102,0.005) 100%)`;
  }
