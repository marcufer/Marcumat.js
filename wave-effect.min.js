/**
 * wave-effect.ultra-smooth.js
 * - Ripple effect, ultra smooth even on large elements or during scrolling
 * - Uses canvas for large elements, passive event listeners, GPU compositing, containment
 * - Fallbacks to DOM span for small/normal elements
 * - API/behavior, UI, and function parity with original, but lighter and higher quality
 */
...
(function(root, factory) {
  ...
  const MAX_EXTRA = 0.2;
  const MIN_RIPPLE_INTERVAL = 100;
  const GLOBAL_MIN_RIPPLE_INTERVAL = 60;
  const GLOBAL_MAX_RIPPLES = 7;
  let RIPPLE_FADE_DURATION_AUTO = 650;
  let RIPPLE_FADE_DURATION_MANUAL = 380;
  ...
  let lastScrollTime = 0, isRapidScrolling = false, scrollThreshold = 60;
  window.addEventListener('scroll', () => {
    const now = performance.now();
    if (now - lastScrollTime < scrollThreshold) {
      isRapidScrolling = true;
      clearTimeout(window._rapidScrollTimer);
      window._rapidScrollTimer = setTimeout(() => { isRapidScrolling = false; }, scrollThreshold * 1.7);
    }
    lastScrollTime = now;
  }, {passive:true});
  ...
  const SCROLL_VELOCITY_THRESHOLD = 1.45, SCROLL_DISTANCE_THRESHOLD = 15, SCROLL_TIME_WINDOW = 65, FINE_SCROLL_THRESHOLD = 0.40;
  ...
  // Ripple pool
  const MAX_RIPPLES_PER_ELEMENT = 8;
  ...
  // Large element detection and canvas ripple
  function isLargeElement(rect) {
    return rect.width > 400 || rect.height > 400;
  }
  function getCanvasRipple(el, x, y, radius, color, duration) {
    const scale = window.devicePixelRatio || 1;
    const canvas = document.createElement('canvas');
    canvas.width = Math.round(el.offsetWidth * scale);
    canvas.height = Math.round(el.offsetHeight * scale);
    ...
    // Animate fade out
    setTimeout(() => {
      canvas.style.transition = `opacity ${duration}ms cubic-bezier(0.4,0,0.2,1)`;
      canvas.style.opacity = '0';
      setTimeout(() => { if (canvas.parentNode) canvas.parentNode.removeChild(canvas); }, duration + 80);
    }, 24);
    return canvas;
  }
  ...
  function onPointerDown(e) {
    ...
    scheduleRipple(() => {
      ...
      const duration = isLargeElement(rect) ? BASE_DURATION - 70 : smartDuration(x, y, rect.width, rect.height);
      const colorVal = getRippleColor(el) || 'rgba(40,170,250,0.35)';
      if (isLargeElement(rect)) {
        // ใช้ canvas ripple
        const canvasRipple = getCanvasRipple(el, x, y, radius, colorVal, duration);
        el.appendChild(canvasRipple);
        activeRipples.set(el, canvasRipple);
        setTimeout(() => { activeRipples.set(el, null); }, duration + 100);
      } else {
        // ปกติ (ใช้ DOM span ripple)
        ...
        ripple.style.setProperty('--ripple-final-scale', (radius * 2) / RIPPLE_HALO_START_DIAMETER);
        ...
        requestAnimationFrame(()=>{
          ...
          animateRipple(ripple, (radius * 2) / RIPPLE_HALO_START_DIAMETER);
