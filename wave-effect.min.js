/**
 * wave-effect (long-press UI glitch fixes)
 *
 * Fixes applied:
 * - Removed per-node 'contain: strict' usage and avoid toggling display to reduce repaint/layer churn.
 * - Removed synthetic event dispatch (TouchEvent/PointerEvent) when forwarding parent end events to delegate.
 *   Instead, parent end handler fades ripples on the delegate target directly.
 * - Kept all main behaviors (ripple creation, expansion, fade, delegate support) intact.
 */
(function (root, factory) {
  ...
  function getRippleNode(el) {
    ...
    // clear inline styles to avoid leftover properties; avoid contain/display toggles
    node.style.cssText = '';
    return node;
  }
  function releaseRippleNode(el, node) {
    ...
    // remove from DOM and recycle. Do NOT toggle display to avoid layout churn.
  ...
  function fadeOutAndRemoveRipple(ripple, el) {
    if (!ripple) return;
  ...
  // Delegate ripple support — but forward "end" by fading delegate ripples directly (no synthetic events)
    ...
    while (el && el !== document.body) {
      if (el.hasAttribute && el.hasAttribute('wave-delegate')) {
        ...
        if (delegateTarget && delegateTarget.hasAttribute('wave')) {
          ...
          // Forward end by fading ripples on delegateTarget directly (no synthetic DOM events)
          function forwardEndRipple(ev) {
            if (!delegateTarget._ripple_delegate_active) return;
            delegateTarget._ripple_delegate_active = false;
            // Fade any active ripples for the delegate target
            try {
  ...
  const config = {
    ...
    setFadeDuration(ms) { if (typeof ms === 'number' && ms > 50 && ms < 5000) RIPPLE_FADE_DURATION = ms | 0; },
    setRippleDuration(ms) { /* API placeholder */ }
  };
