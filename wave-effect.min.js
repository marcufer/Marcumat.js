/**
 * wave-effect.js (now loads wave-effect.min.css from /assets only)
 * Pro Material Ripple Effect (professional: tight center, smooth dynamic halo, visually clear wave-out)
 * @version 2.3.6
 * (improved: ripple color and opacity now controlled only in JS, default is a bit less transparent for clearer effect)
 */
...
(function(root, factory) {
  ...
  const COVERAGE_EXPAND_RATIO = 0.1;
  ...
  // Ripple color and opacity control (default: now a bit less transparent for clarity)
    ...
    if (color) {
      if (/^rgba?\([^)]+\)$/i.test(color) || /^hsla?\([^)]+\)$/i.test(color)) {
        ...
        if (inside) {
          ...
          // Now a bit less transparent than before (core: 0.095, halo: 0.047, edge: 0.006)
          let p1 = Math.min(1, Math.max(alpha, 0.095)); // core
          let p2 = Math.max(0, 0.5 * p1);               // halo
          let p3 = Math.max(0, 0.063 * p1);             // outer edge
      ...
      return `radial-gradient(circle, ${color}18 62%, ${color}0B 74%, ${color}01 100%)`;
    }
    // default system ripple (a bit less transparent blue-gray)
    return `radial-gradient(circle, rgba(24,68,102,0.095) 62%, rgba(24,68,102,0.047) 74%, rgba(24,68,102,0.006) 100%)`;
  }
    ...
    ripples.forEach(r => {
      ...
      function removeRippleAfterFade(e) {
        ...
        if (!e || e.propertyName === "opacity") {
          removed = true;
          r.removeEventListener('transitionend', removeRippleAfterFade);
          r.remove();
          globalActiveWavesCount = Math.max(0, globalActiveWavesCount-1);
        }
      }
      r.addEventListener('transitionend', removeRippleAfterFade);
      setTimeout(removeRippleAfterFade, parseInt(r.style.getPropertyValue('--ripple-fade-duration')) || 1200);
    });
  ...
  function onPointerDown(e) {
    ...
    function endRipple(){
      const now = performance.now();
      if (animationEnded || (now - rippleCreatedAt) >= duration - 10) {
        ripple.style.setProperty('--ripple-fade-duration', '800ms');
      }
      ripple.classList.add('fading');
      let removed = false;
      function removeRippleAfterFade(ev) {
        if (removed) return;
      ...
      setTimeout(removeRippleAfterFade, parseInt(ripple.style.getPropertyValue('--ripple-fade-duration')) || 1200);
      activeRipples.set(el, null);
