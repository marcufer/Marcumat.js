/**
 * wave-effect.ultra-optimized.js (improved for minimal resource & ultra smoothness, scroll-aware, gesture-aware)
 * - Prevents ripple during rapid scroll or scroll-like gestures
...
(function(root, factory) {
  ...
  // --- Scroll detection: prevents ripple if user is scrolling fast
  ...
  // --- Gesture/Scroll Touch Detection (NEW)
  let lastTouchTime = 0;
  let lastTouchY = 0, lastTouchX = 0;
  let lastTouchMoveY = 0, lastTouchMoveX = 0;
  let touchMoved = false;
  let touchStartTime = 0;
  let touchMoveHistory = [];
  let ignoreNextRipple = false;

  // Gesture/Scroll Speed Detection Parameters
  const SCROLL_VELOCITY_THRESHOLD = 1.7; // px/ms (tune as needed)
  const SCROLL_DISTANCE_THRESHOLD = 18; // px, minimal move = scroll
  const SCROLL_TIME_WINDOW = 80; // ms, lookback window for velocity
  const FINE_SCROLL_THRESHOLD = 0.43; // px/ms, slow gentle scroll
  ...
  function onTouchMove(e) {
    const t = e.touches?.[0] || e;
    const now = performance.now();
    const dx = t.clientX - lastTouchMoveX;
    const dy = t.clientY - lastTouchMoveY;
    const dist = Math.hypot(dx, dy);
    touchMoved = touchMoved || (dist > SCROLL_DISTANCE_THRESHOLD);
    lastTouchMoveX = t.clientX;
    lastTouchMoveY = t.clientY;
    // Store history for velocity calculation
    touchMoveHistory.push({t: now, x: t.clientX, y: t.clientY});
    if (touchMoveHistory.length > 8) touchMoveHistory.shift();
    // Calculate velocity
    let baseIdx = 0;
    for (let i = touchMoveHistory.length-2; i >= 0; i--) {
      if (now - touchMoveHistory[i].t > SCROLL_TIME_WINDOW) {
        baseIdx = i; break;
      }
    }
    const base = touchMoveHistory[baseIdx];
    const main = touchMoveHistory[touchMoveHistory.length-1];
    const dt = main.t - base.t;
    if (dt > 10) {
      const vx = (main.x - base.x)/dt;
      const vy = (main.y - base.y)/dt;
      const v = Math.hypot(vx, vy);
      if (v > SCROLL_VELOCITY_THRESHOLD) {
        ignoreNextRipple = true;
      }
    }
  }
  function onTouchEnd(e) {
    if (touchMoved) {
      if (touchMoveHistory.length >= 2) {
        const first = touchMoveHistory[0];
        const last = touchMoveHistory[touchMoveHistory.length-1];
        const dt = last.t - first.t;
        const dist = Math.hypot(last.x - first.x, last.y - first.y);
        const v = dist/dt;
        if (v > FINE_SCROLL_THRESHOLD) {
          ignoreNextRipple = true;
        }
      } else {
        ignoreNextRipple = true;
  ...
  // --- Main pointer handler (improved: check ignoreNextRipple)
