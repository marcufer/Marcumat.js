 ...
 * - Center "dot" and near area always small and fixed (NOT dynamic, e.g. 18px).
 * - The "halo"/outer edge is the only part that expands to cover dynamic size.
 * - Animation: starts as a tight dot (fixed size), then a soft halo grows outwards and covers the element.
 * - The "wave" is visually clear as it moves outward from the touch point.
 * - All fade/blur, accessibility, multify, and config logic preserved.
 * @version 2.3.1
 */
...
(function(root, factory) {
  ...
  // --- Ripple geometry ---
  const RIPPLE_CENTER_DIAMETER = 18; // px, always fixed for center and near area
  const RIPPLE_HALO_START_DIAMETER = 18; // px, always fixed at start
  const RIPPLE_HALO_OPACITY = 0.14;  // main halo alpha at start
  const RIPPLE_HALO_END_OPACITY = 0.008; // outermost edge alpha
  const RIPPLE_HALO_BLUR = 0.8; // px
  ...
  function autoLoadWaveEffectCSS() {
    ...
    for (const link of document.querySelectorAll('link[rel="stylesheet"]')) {
      if (link.href && /wave-effect\.min\.css([?#].*)?$/i.test(link.href)) {
    ...
    const candidates = [
      '/wave-effect.min.css',
      '/css/wave-effect.min.css',
      '/assets/wave-effect.min.css'
    ];
    ...
    if (thisScript && thisScript.src) {
      try {
        ...
        url.pathname = url.pathname.replace(/[^/]+$/, 'wave-effect.min.css');
  ...
  function getRippleBg(color) {
    // Center: 0.20, mid: 0.10, edge: 0.008 (main halo)
    if (color) {
      if (/^rgba?\([^)]+\)$/i.test(color) || /^hsla?\([^)]+\)$/i.test(color)) {
        ...
        if (inside) {
          ...
          let p1 = Math.min(1, Math.max(alpha, 0.20)); // center
          let p2 = Math.max(0, 0.50 * p1); // mid (0.10)
          let p3 = Math.max(0, 0.04 * p1); // edge (0.008)
      ...
      // For hex: center (33), mid (19), edge (02)
      return `radial-gradient(circle, ${color}33 62%, ${color}19 74%, ${color}02 100%)`;
    }
    // Default: white, alpha 0.20, 0.10, 0.008
  ...
  function maximalExpandedCoverageRadius(x, y, width, height) {
    // Only used for the outer halo, not the center
    let dists = [
