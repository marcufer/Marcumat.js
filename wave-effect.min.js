/**
 * wave-effect.ultra-opt-lite.js
 * - Resource usage further minimized, ultra smooth, scroll-aware, fully configurable scroll threshold
 * - All features and quality preserved
...
(function(root, factory) {
  ...
  let scrollThreshold = 80; // ms, default
  function setScrollThreshold(ms) {
    if (typeof ms === 'number' && ms >= 20 && ms <= 1000) scrollThreshold = ms|0;
  }

  // --- State
  const RIPPLE_CLASS = 'ripple';
  const SURFACE_CLASS = 'ripple-surface';
  const BASE_DURATION = 490, MIN_DURATION = 370, MAX_EXTRA = 0.17;
  const MIN_RIPPLE_INTERVAL = 150, GLOBAL_MIN_RIPPLE_INTERVAL = 100, GLOBAL_MAX_RIPPLES = 7;
  let RIPPLE_FADE_DURATION_AUTO = 800, RIPPLE_FADE_DURATION_MANUAL = 500;
  const RIPPLE_HALO_START_DIAMETER = 18, RIPPLE_HALO_BLUR = 0.8, COVERAGE_EXPAND_RATIO = 0.2;

  let lastGlobalRippleTime = 0, globalActiveWavesCount = 0;
  const lastAllowed = new WeakMap(), activeRipples = new WeakMap();
  let waveSettingLoaded = false, waveJsonLoaded = false, waveEffectCSSLoaded = false;
  const settings = { disableTapHighlight: false };
  const upgradedElements = new WeakSet();
  let lastUpgradeAll = 0, cachedWaveNodes = [];

  // --- Scroll detection with debounced flag
  let lastScrollTime = 0, isRapidScrolling = false;
  ...
  window.addEventListener('scroll', () => {
    ...
    if (now - lastScrollTime < scrollThreshold) {
      ...
      scrollTimer = setTimeout(() => { isRapidScrolling = false; }, scrollThreshold * 2);
    }
    lastScrollTime = now;
  }, {passive: true});
  ...
  function autoLoadWaveSettingJSON(callback) {
    ...
    (function tryNext(idx){
      ...
      fetch(candidates[idx], {method:'GET', credentials:'same-origin'})
        ...
        .then(json => {
          if (json && typeof json === 'object') { waveJsonLoaded = true; Object.assign(settings, json); if (callback) callback(settings); }
          else { tryNext(idx+1); }
        })
        .catch(()=>tryNext(idx+1));
    })(0);
  ...
  function getRippleBg(color) {
    ...
    return `radial-gradient(circle, rgba(${defaultC[0]},${defaultC[1]},${defaultC[2]},0.13) 62%, rgba(${defaultC[0]},${defaultC[1]},${defaultC[2]},0.078) 74%, rgba(${defaultC[0]},${defaultC[1]},${defaultC[2]},0.028) 100%)`;
  }
  ...
  // --- Minimized getBoundingClientRect usage (cache, invalidate on scroll)
  ...
  window.addEventListener('scroll', () => { rectCache.clear && rectCache.clear(); });
  ...
  function querySelectorAllCached(sel) {
    const now = Date.now();
    if (sel === '[wave]' && (now - lastUpgradeAll < 2500) && cachedWaveNodes.length > 0) return cachedWaveNodes;
    const nodes = Array.from(document.querySelectorAll(sel));
    if (sel === '[wave]') { cachedWaveNodes = nodes; lastUpgradeAll = now; }
    return nodes;
  }
  ...
  const config = {
    ...
    get scrollThreshold() { return scrollThreshold; },
