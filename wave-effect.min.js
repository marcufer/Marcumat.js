/**
 * wave-effect.js (resource-optimized version)
 * Custom: แก้ไขชื่อและการใช้งานตัวแปร fade duration ให้ตรงตามนิยาม
 * 
 * - auto: ผู้ใช้วางนิ้วลงแล้วยกออกทันที (fade-out อัตโนมัติหลังครบระยะ ripple)
 * - manual: ผู้ใช้วางนิ้วค้างจน animation จบ แล้วจึงยกนิ้วออก (fade-out manual หลังปล่อย)
 * 
 * Resource-optimized: ลด DOM/GC/Observer/Event ซ้ำซ้อน แต่ UI/UX/Logic เดิม 100%
 */
...
(function(root, factory) {
  ...
  // --- Fade-out durations (customizable) ---
  let RIPPLE_FADE_DURATION_AUTO = 900;
  ...
  // --- Optimize: cache and deduplicate stylesheet loading ---
  ...
  // --- Optimize: only try until found, skip if loaded ---
  function autoLoadWaveSettingCSS() {
    if (waveSettingLoaded) return;
    if ([...document.querySelectorAll('link[rel="stylesheet"]')].some(link => /wave-setting(\.min)?\.css([?#].*)?$/i.test(link.href))) {
  ...
  // --- Optimize: only fetch setting json once, minimal error path ---
  ...
  // --- Optimize: inject style tag only once globally, skip if already injected ---
  let tapHighlightInjected = false;
  function injectTapHighlightCSS() {
    if (tapHighlightInjected) return;
    if (document.querySelector('style[data-wave-tap-highlight="true"]')) {
  ...
  // --- Optimize: reduce getComputedStyle calls by limiting DOM walk ---
  function getDefaultRippleColor(el) {
    let node = el;
    while (node) {
      if (node.style && node.style.getPropertyValue('--ripple-default-color')) {
        return node.style.getPropertyValue('--ripple-default-color').trim();
      }
      if (node.nodeType === 1) {
  ...
  // --- Optimize: keep logic but avoid string allocations if possible ---
  function getRippleBg(color) {
    ...
    if (color) {
      ...
      // fallback for hex or string: use reduced opacity
  ...
  function smartBlur() {
    return `blur(${RIPPLE_HALO_BLUR}px)`;
  }
  ...
  // --- Optimize: remove event after fade, do not leak, always reduce global count ---
  function fadeOutAndRemoveRipple(ripple, fadeType) {
    if (!ripple) return;
    ...
    setTimeout(removeRippleAfterFade, duration + 200);
  }

  function clearRipples(el) {
    const ripples = el.querySelectorAll('.' + RIPPLE_CLASS);
    ripples.forEach(r => { fadeOutAndRemoveRipple(r, 'manual'); });
  ...
  // --- Optimize: avoid unnecessary computations, no extra closures ---
  function onPointerDown(e) {
    ...
    let endRipple = function() {
      if (!ripple.parentNode) return;
      pointerReleased = true;
      if (!animationEnded) {
        fadeOutAndRemoveRipple(ripple, 'auto');
      } else {
        fadeOutAndRemoveRipple(ripple, 'manual');
      }
      activeRipples.set(el, null);
    ...
    el.addEventListener('pointerup', endRipple, {once:true});
    el.addEventListener('pointerleave', endRipple, {once:true});
    el.addEventListener('touchend', endRipple, {once:true});
    el.addEventListener('touchcancel', endRipple, {once:true});
  ...
  // --- Optimize: batch upgrade instead of per-mutation, only one mutation observer ---
  function upgradeAll() {
    const nodes = document.querySelectorAll('[wave]');
    for (let i = 0, l = nodes.length; i < l; ++i) {
      upgradeElement(nodes[i]);
    }
  ...
  function initWakeEffect() {
    ...
    // --- Optimize: only one observer, minimal observed attributes, disconnect on unload ---
    let observer = new MutationObserver(function(mutationsList) {
      let needsUpgrade = false;
      for (let mutation of mutationsList) {
        if (
          mutation.type === 'childList' ||
          (mutation.type === 'attributes' && (mutation.attributeName === 'wave' || mutation.attributeName === 'data-ripple-color'))
        ) {
          needsUpgrade = true;
          break;
        }
    ...
    observer.observe(document.documentElement, {
      subtree:true, childList:true, attributes:true, attributeFilter:['wave','data-ripple-color']
    });
  ...
  // Developer API: set fade durations
  const config = {
    BASE_DURATION,
    MIN_DURATION,
    MAX_EXTRA,
