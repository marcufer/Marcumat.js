/**
 * Improved wave-effect.js
 * - เหมือนเดิมทุกประการทั้ง UI, logic, สี, ฟีล, timing
 * - ใช้ทรัพยากรน้อยลงและรันลื่นกว่าเดิม
 */
(function(root, factory) {
  ...
  // --- Configurable main parameters (เหมือนเดิม) ---
  ...
  // -- Utility cache --
  const _qs_cache = new WeakMap();

  function querySelectorAllCached(sel) {
    // Cheap cache for fast DOM scan in upgradeAll
    let c = _qs_cache.get(document);
    if (!c || c.sel !== sel) {
      const nodes = Array.from(document.querySelectorAll(sel));
      _qs_cache.set(document, { sel, nodes, stamp: Date.now() });
      return nodes;
    }
    return c.nodes;
  }

  // --- CSS Loader (เดิม, optimize document.head append และ onload) ---
  ...
  function getRippleBg(color) {
    ...
    return `radial-gradient(circle, rgba(${defaultC[0]},${defaultC[1]},${defaultC[2]},0.13) 62%, rgba(${defaultC[0]},${defaultC[1]},${defaultC[2]},0.078) 74%, rgba(${defaultC[0]},${defaultC[1]},${defaultC[2]},0.018) 100%)`;
  }

  // ใช้ Math.hypot แค่รอบเดียวต่อหนึ่ง ripple
  ...
  // --- Optimized pointerdown handler (no closure, single function reused) ---
  function onPointerDown(e) {
    ...
    // event handler reuse (no closure)
    function endRipple() {
  ...
  // -- Optimized keydown handler (no closure, single function reused) --
  ...
  // --- Upgrade/observe logic: Only observe new nodes (no full scan every mutation) ---
  ...
  function upgradeAll() {
    // ใช้ cache selector ถ้ามี
    const nodes = querySelectorAllCached('[wave]');
    for (let el of nodes) upgradeElement(el);
    if (settings.disableTapHighlight) injectTapHighlightCSS();
  }

  // Observe เฉพาะ childList และ attribute ที่เกี่ยวข้องเท่านั้น
  let observer;
  function observeDom() {
    if (observer) return;
      ...
      for (const m of muts) {
        if (m.type === 'childList') {
          m.addedNodes && m.addedNodes.forEach(n=>{
            if (n.nodeType === 1 && n.hasAttribute && n.hasAttribute('wave')) upgradeElement(n);
            if (n.querySelectorAll) n.querySelectorAll && n.querySelectorAll('[wave]').forEach(upgradeElement);
          });
        } else if (m.type === 'attributes' && m.target && m.attributeName === 'wave') {
          upgradeElement(m.target);
        }
      }
    });
