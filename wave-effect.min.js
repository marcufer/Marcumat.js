/**
 * wave-effect.ultra-optimized.js (Same UI/logic, even lower RAM/GPU/CPU usage)
 * All behavioral parity - user will not notice any change!
 */
(function(root, factory) {
  ...
  // Minor optimization: use document.querySelectorAll only if cache expired
  function querySelectorAllCached(sel) {
    const time = Date.now();
    if (sel === '[wave]' && (time - lastUpgradeAll < 1800) && cachedWaveNodes.length)
      return cachedWaveNodes;
    // Minor: Use static NodeList, avoid Array allocation unless needed
    const nodes = cachedWaveNodes.length ? cachedWaveNodes : Array.from(document.querySelectorAll(sel));
    if (sel === '[wave]') {
  ...
  // --- Scroll/Touch Prevention ---
  // Optimize: only one scroll listener, minimal work
  ...
  // Minor: reuse touch object, minimize allocations
  function onTouchStart(e) {
    ...
    touchMoveHistory.length = 0; // clear array in place
  ...
  function scheduleRipple(fn) {
    ...
    if (!rippleRAF) {
      rippleRAF = requestAnimationFrame(() => {
        // Minor optimization: for loop instead of for-of
  ...
  function releaseRippleNode(el, node) {
    ...
    requestAnimationFrame(() => {
      ...
      // Minor: only remove if parent is correct to avoid layout thrash
  ...
  // --- CSS Loader ---
  // Minor: only create <link> if not already present
  ...
  // Minor: only tryNext if not loaded
  ...
  function allowGlobalRipple() {
    const n = now();
    if (n - lastGlobalRippleTime < GLOBAL_MIN_RIPPLE_INTERVAL) return false;
    if (globalActiveWavesCount >= GLOBAL_MAX_RIPPLES) return false;
    lastGlobalRippleTime = n;
  ...
  function onPointerDown(e) {
    ...
    const n = now(), last = lastAllowed.get(el) || 0;
    if (n - last < MIN_RIPPLE_INTERVAL) {
    ...
    scheduleRipple(() => {
      const rect = getRect(el),
        ...
        duration = smartDuration(x, y, rect.width, rect.height),
