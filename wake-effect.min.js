/**
 * wake-effect.js
 * Pro Material Ripple Effect (extremely transparent: center, mid, edge all lighter)
 * - Center stop: lighter (alpha 0.25)
 * - Middle stop: very transparent (alpha 0.025)
 * - Edge stop: ultra transparent (alpha 0.005)
 * - Extremely tight, light, smooth ripple effect
 * @version 2.2.10
 * @license MIT
 */
...
(function(root, factory) {
  ...
  // --- Core logic (all stops ultra transparent) ---
  ...
  function getRippleBg(color) {
    // Center: 0.25, mid: 0.025, edge: 0.005
    if (color) {
      if (/^rgba?\([^)]+\)$/i.test(color) || /^hsla?\([^)]+\)$/i.test(color)) {
        ...
        if (inside) {
          ...
          let p1 = Math.min(1, Math.max(alpha, 0.25)); // center
          let p2 = Math.max(0, 0.1 * p1); // mid (0.025)
          let p3 = Math.max(0, 0.02 * p1); // edge (0.005)
      ...
      // For hex: center (40), mid (06), edge (01)
      return `radial-gradient(circle, ${color}40 50%, ${color}06 86%, ${color}01 100%)`;
    }
    // Default: white, alpha 0.25, 0.025, 0.005
    return `radial-gradient(circle, rgba(255,255,255,0.25) 50%, rgba(255,255,255,0.025) 86%, rgba(255,255,255,0.005) 100%)`;
  }
